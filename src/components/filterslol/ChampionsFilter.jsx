...
const ChampionsFilter = ({
  ...
  const [maxChampionInput, setMaxChampionInput] = useState("171");

  const updateSliderChampionsProgress = (minVal, maxVal) => {
    const minPercentChampions = (minVal / 171) * 100;
    const maxPercentChampions = (maxVal / 171) * 100;
  ...
  const handleMaxChampionChange = (e) => {
    ...
    } else if (inputValue > 171) {
      setMaxChampionInput("171");
      onChampionChange(minChampion, 171);
    } else {
      const value = Number(inputValue);
      if (value >= minChampion && value <= 171) {
